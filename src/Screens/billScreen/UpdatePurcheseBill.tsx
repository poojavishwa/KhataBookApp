...
import { UpdatePurcheseBillById } from "../../Api/billCrud/BillCrud";
import ProductPurcheseModal from "../ProductsScreen/ProductPurcheseModal";
import { showToast } from "../../constants/showToast";

const UpdatePurcheseBill = () => {
  const route = useRoute();
  const { purcheseBillData } = route.params as { purcheseBillData: any };
  const navigation = useNavigation();
  const [billNumber, setBillNumber] = useState(1);
  const [date, setDate] = useState(new Date());
  const [showDatePicker, setShowDatePicker] = useState(false);
  const [selectedProducts, setSelectedProducts] = useState<{ name: string; quantity: number;price:number; costPrice: number; productId?: string }[]>([]);
  const [paymentMethod, setPaymentMethod] = useState("Cash");
  const [modalVisible, setModalVisible] = useState(false);
  const [customerModalVisible, setCustomerModalVisible] = useState(false);
  const [selectedCustomer, setSelectedCustomer] = useState<any>("");
  const [isSaving, setIsSaving] = useState(false);
  console.log("purcheseBillData",purcheseBillData)
  useEffect(() => {
    if (purcheseBillData && Object.keys(purcheseBillData).length > 0) {
      setBillNumber(purcheseBillData.BillNumber || "1");
      setDate(purcheseBillData.Date ? new Date(purcheseBillData.Date) : new Date());
      setSelectedProducts(
        Array.isArray(purcheseBillData.items)
          ? purcheseBillData.items.map((item) => ({
      ...
      setPaymentMethod(purcheseBillData.paymentStatus || "Cash");
      setSelectedCustomer(purcheseBillData.supplierId || "");
    }
  }, [purcheseBillData]);

  const totalAmount = selectedProducts.reduce((total, item) => total + item.quantity * item.price, 0);
  const billId = purcheseBillData._id;
 ...
 const updateBill = async () => {
  ...
  const billData = {
    billNumber,
    date: formattedDate, 
    purchaseBillAmount:totalAmount,
  ...
  console.log("Updated Bill Data:", billData, "Bill ID:", purcheseBillData._id);

  try {
    await UpdatePurcheseBillById(billData, billId);
    console.log("Updated Bill Data:", billData, "Bill ID:", purcheseBillData._id);
    showToast("error","Success", "Bill updated successfully!");
    navigation.navigate("Purchese Invoice", {
      billId,
      billNumber,
    ...
    <>
      <View style={{ padding: 20 }}>
        <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginTop: 5 }}>
          <View>
            <Text style={{ fontSize: 14 }}>Purchase Bill No:      </Text>
          ...
          <View>
            ...
            <TouchableOpacity onPress={() => setShowDatePicker(true)} style={{ flexDirection: "row", alignItems: "center", borderWidth: 1, padding: 10,borderColor:"#D0DDD0" }}>
        ...
        <View style={styles.totalContainer}>
          <Text style={styles.totalText}>Purchase Bill Amount:</Text>
...
export default UpdatePurcheseBill;
